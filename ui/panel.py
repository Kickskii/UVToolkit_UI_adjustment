...
class UVTOOLKIT_PT_unwrap(Panel):
    ...
    def draw(self, context):
        ...
        row.operator("uv.toolkit_straighten_island", icon_value=icons_coll["straighten_island"].icon_id)
        row = col.row(align=True)
        row.operator('uv.toolkit_straighten', icon_value=icons_coll["straighten"].icon_id).gridify = False
        row.operator('uv.toolkit_straighten', text="Gridify", icon_value=icons_coll["gridify"].icon_id).gridify = True


class UVTOOLKIT_PT_tools(Panel):
    bl_label = "Tools"
    bl_idname = "UVTOOLKIT_PT_tools"
    bl_parent_id = "UVTOOLKIT_PT_unwrap"
    bl_space_type = 'IMAGE_EDITOR'
    bl_region_type = 'UI'
    ...
    def draw(self, context):
        ...
        # Row 1
        row = col.row(align=True)
        row.operator("uv.toolkit_sharp_edges_from_uv_islands", icon_value=icons_coll["sharp_edges_from_uv_islands"].icon_id)
        prop = row.operator("uv.seams_from_islands", icon_value=icons_coll["seams_from_islands"].icon_id)
        ...
        # Row 3
        row = col.row(align=True)
        row.operator("uv.toolkit_split_faces_move", text="Split Faces", icon_value=icons_coll["split_faces_move"].icon_id)
        row.operator("uv.toolkit_clear_all_seams", icon_value=icons_coll["clear_all_seams"].icon_id)
        
        # Row 4
        col.operator("uv.toolkit_udim_packing", icon_value=icons_coll["udim_packing"].icon_id)


# ==========================================
# 2. TRANSFORM, ALIGN & ARRANGE (MAIN PANEL)
# ==========================================
...
class UVTOOLKIT_PT_transform(Panel):
    ...
    def draw(self, context):
        ...
        # --- Move D-Pad (Preserved exactly as requested) ---
        ...
        col.label(text="Move")
        col.operator("transform.translate", text="", icon_value=icons_coll["move_left"].icon_id).value = (distance * -1, 0, 0)
        col.label(text="")
        
        col = row.column(align=True)
        col.operator("transform.translate", text="", icon_value=icons_coll["move_up"].icon_id).value = (0, distance, 0)
        col.prop(scene.uv_toolkit, "move_distance", text="")
        col.operator("transform.translate", text="", icon_value=icons_coll["move_down"].icon_id).value = (0, distance * - 1, 0)
        
        col = row.column(align=True)
        col.label(text="")
        col.operator("transform.translate", text="", icon_value=icons_coll["move_right"].icon_id).value = (distance, 0, 0)
        col.label(text="")

        layout.separator()

        # --- Rotate & Scale Compact Block ---
        block = layout.column(align=True)
        
        # Rotate sub-block
        row = block.row(align=True)
        row.prop(scene.uv_toolkit, "island_rotation_angle", text="Rotate")
        row.prop(scene.uv_toolkit, "island_rotation_mode", expand=True)
        
        row = block.row(align=True)
        angle = scene.uv_toolkit.island_rotation_angle
        row.operator("uv.toolkit_rotate_islands", text=f"-{angle}°", icon_value=icons_coll["rotate_ccw"].icon_id).cw = False
        row.operator("uv.toolkit_rotate_islands", text=f"{angle}°", icon_value=icons_coll["rotate_cw"].icon_id).cw = True
        
        block.separator(factor=0.5)
        
        # Scale sub-block
        row = block.row(align=True)
        row.prop(scene.uv_toolkit, "island_scale_x", text="Scale X")
        row.prop(scene.uv_toolkit, "island_scale_y", text="Y")
        
        row = block.row(align=True)
        row.operator("uv.toolkit_scale_islands", icon_value=icons_coll["scale"].icon_id)
        ...
        row.operator("transform.mirror", text="Flip X", icon_value=icons_coll["mirror_x"].icon_id).constraint_axis = (True, False, False)
        row.operator("transform.mirror", text="Flip Y", icon_value=icons_coll["mirror_y"].icon_id).constraint_axis = (False, True, False)


class UVTOOLKIT_PT_align(Panel):
    bl_label = "Align"
    bl_idname = "UVTOOLKIT_PT_align"
    bl_parent_id = "UVTOOLKIT_PT_transform"
    bl_space_type = 'IMAGE_EDITOR'
    bl_region_type = 'UI'
    bl_options = {'DEFAULT_CLOSED'}
...
class UVTOOLKIT_PT_arrange(Panel):
    ...
    def draw(self, context):
        ...
        row.operator("uv.toolkit_stack_similar_islands", text="Stack Similar", icon_value=icons_coll["stack_similar_islands"].icon_id)
        ...
        row = col.row(align=True)
        row.operator("uv.toolkit_randomize_islands", text="Randomize", icon_value=icons_coll["randomize_islands"].icon_id)
        row.operator("uv.toolkit_fit_to_bounds", icon_value=icons_coll["fit_to_bounds"].icon_id)


# ==========================================
# 3. SELECT & PINS (MAIN PANEL)
# ==========================================
...
class UVTOOLKIT_PT_select(Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column(align=True)
        row = col.row(align=True)
        row.operator("uv.toolkit_select_island_border", text="Island border", icon_value=icons_coll["select_island_border"].icon_id)
        row.operator("uv.toolkit_select_similar_islands", text="Similar Islands", icon_value=icons_coll["select_similar_islands"].icon_id)
        
        row = col.row(align=True)
        row.operator("uv.toolkit_invert_selection", icon_value=icons_coll["invert_selection"].icon_id)
        row.operator("uv.toolkit_select_flipped_islands", text="Flipped Islands", icon_value=icons_coll["select_flipped_islands"].icon_id)
        
        col.operator("uv.toolkit_find_udim_crossing", icon_value=icons_coll["find_udim_crossing"].icon_id)
        col.operator("uv.toolkit_find_shattered_islands", icon_value=icons_coll["find_shattered_islands"].icon_id)


class UVTOOLKIT_PT_pins(Panel):
    bl_label = "Pins"
    bl_idname = "UVTOOLKIT_PT_pins"
    bl_parent_id = "UVTOOLKIT_PT_select"
    bl_space_type = 'IMAGE_EDITOR'
    bl_region_type = 'UI'
    bl_options = {'DEFAULT_CLOSED'}
    ...
    def draw(self, context):
        ...
        col.operator("uv.toolkit_clear_all_pins", icon_value=icons_coll["clear_all_pins"].icon_id)


# ==========================================
# 4. DISPLAY, MAPS & SETTINGS (MAIN PANEL)
# ==========================================
...
class UVTOOLKIT_PT_display(Panel):
    ...
    def draw(self, context):
        ...
        col.operator("uv.toolkit_toggle_material", icon_value=icons_coll["toggle_material"].icon_id)
        col.operator("uv.toolkit_toggle_color_mode", icon_value=icons_coll["toggle_color_mode"].icon_id)
        col.operator("uv.toolkit_toggle_grid_type", icon_value=icons_coll["toggle_grid_type"].icon_id)


class UVTOOLKIT_PT_uv_sync(Panel):
    bl_label = "UV Sync"
    bl_idname = "UVTOOLKIT_PT_uv_sync"
    bl_parent_id = "UVTOOLKIT_PT_display"
    bl_space_type = 'IMAGE_EDITOR'
    bl_region_type = 'UI'
    bl_options = {'DEFAULT_CLOSED'}
    ...
    def draw(self, context):
        layout = self.layout
        col = layout.column(align=True)
        col.operator("uv.toolkit_sync_mode", text="Toggle UV Sync", icon='UV_SYNC_SELECT')
...
class UVTOOLKIT_PT_uv_sync_settings(Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column(align=True)
        col.label(text="Selected Elements:")
        col.prop(addon_prefs, "sync_selection", expand=True)
        col.separator()
        col.label(text="Selection Mode:")
        col.prop(addon_prefs, "sync_uv_selction_mode", expand=True)
...
class UVTOOLKIT_PT_checker_map(Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column(align=True)
        col.prop(addon_prefs, "checker_map", expand=True)
        ...
        if addon_prefs.checker_map == "BUILT-IN":
            ...
            prop = col.operator("uv.toolkit_create_checker_material", icon_value=icons_coll["create_checker_material"].icon_id)
        ...
        layout.separator()
        layout.operator("uv.toolkit_remove_all_checker_materials", icon_value=icons_coll["remove_all_checker_materials"].icon_id)
...
class UVTOOLKIT_PT_uv_maps(Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column(align=True)
        col.prop(scene.uv_toolkit, "uv_layer_name", text="Name")
        
        split = col.split(factor=0.5)
        split.label(text="UV Set Index:")
        split.prop(scene.uv_toolkit, "uv_layer_index", text="")
        
        row = col.row(align=True)
        row.operator("uv.toolkit_set_active_uv_layer", text="Set Active", icon_value=icons_coll["set_active_uv_layer"].icon_id)
        row.operator("uv.toolkit_create_new_uv_layer", text="Create New", icon_value=icons_coll["create_new_uv_layer"].icon_id)
        
        row = col.row(align=True)
        row.operator("uv.toolkit_rename_uv_layers", text="Rename", icon_value=icons_coll["rename_uv_layers"].icon_id)
        row.operator("uv.toolkit_delete_uv_layer", text="Delete", icon_value=icons_coll["delete_uv_layer"].icon_id)
...
class UVTOOLKIT_PT_help(Panel):
    ...
    def draw(self, context):
        icons_coll = get_icons_set(context)
        col = self.layout.column(align=True)
        col.operator("uv.toolkit_open_addon_settings", icon_value=icons_coll["settings"].icon_id)
        col.operator("uv.toolkit_hotkeys", text="List of Hotkeys", icon_value=icons_coll["hotkeys"].icon_id)
        col.operator("wm.url_open", text="Documentation", icon_value=icons_coll["documentation"].icon_id).url = self.documentation
        col.operator("wm.url_open", text="Tutorials", icon_value=icons_coll["tutorials"].icon_id).url = self.tutorials
